...
### GENESIS-040 (Teil 3) – Nachtrag: die Vertexfarben kamen immer an
- 2026-09-22: Der Befund aus Teil 2 („Vertexfarben kommen in Unreal nicht an") war falsch. Nachgemessen an der FBX (Farbschicht `Tissue`, ByPolygonVertex, 1.356.225 Farben) und an den Quelldaten des importierten Meshes: R 0,000…0,996 (M 0,612), G 0,000…1,000 (M 0,331), B 0,000…0,996 (M 0,510) – dieselben Werte wie in Blender. Gleich mit und ohne Nanite, gleich über den klassischen FBX-Weg und über Interchange, gleich bei fünf Testwürfeln (Ecken/Punkte, BYTE/FLOAT, sRGB/linear, FBX/glTF). Gelogen hat die Messung: `EditorStaticMeshLibrary.has_vertex_colors` antwortet im Kommandlet immer `False` – im selben Lauf meldet dieselbe Bibliothek für einen Würfel mit 8 Vertices `get_number_verts = 0`. Jetzt misst `Tools/Unreal/genesis_vertex_colors.py` die MeshDescription über Geometry Script (Plugin nur für den Editor) und meldet Kanäle ohne Spannweite; `Tools/Unreal/show_vertex_colors.py` macht sie im Spiel sichtbar. Im Bild geprüft: Faltenspitzen grün, Spalten magenta/rot. Docs 13, 26, 30.
