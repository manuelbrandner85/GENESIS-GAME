...
def modell_fuer(tabelle, woche):
    ...
    wochen = sorted((float(w) for w in tabelle if float(w) >= 8), key=float)
...
if __name__ == "__main__":
    ...
    # Jede Woche so, wie das Spiel sie zeigt (Momente: 8, 10, 12, 16, 19, 20,5, 24, 28, 31, 34, 37; Drehung 31–33 im Zeitraffer)
    for woche in [w * 0.5 for w in range(16, 81)]:
